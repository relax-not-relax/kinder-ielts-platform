...
import { Avatar, Badge, Button, IconButton, Menu, MenuHandler, MenuItem, MenuList, MobileNav, Navbar, Typography } from '@material-tailwind/react';
    ...
    const navList = (
        <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-12">
            ...
            <Menu allowHover>
                ...
                <MenuList className='p-0 bg-green border-0 rounded-none'>
                    {["khoá học", "khoá học video", "tài liệu"].map((val, index) => {
                        const className = `py-4 px-5 rounded-none text-white ${index === 2
                            ? ""
                            : "border-b border-white"
                            }`;
                        return (
                            <MenuItem key={index} className={className}>
                                <Link className='capitalize text-base font-semibold'>{val}</Link>
                            </MenuItem>
                        );
                    })}
                </MenuList>
